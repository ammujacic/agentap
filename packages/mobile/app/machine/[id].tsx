/**
 * Per-machine sessions drill-down screen
 */
...
export default function MachineSessionsScreen() {
  ...
  const { setMachines } = useMachinesStore();
  const sessions = useSessionsStore((s) => s.sessions);
  const { refreshAll } = useWebSocketContext();
  const api = createApiClient(API_URL, API_HEADERS);
  const machineSessions = useMemo(() => sessions.filter((s) => s.machineId === id), [sessions, id]);
  ...
  const filteredSessions = useMemo(() => {
    ...
    return result;
  }, [machineSessions, search, filter, sort]);
      ...
      {machine && (
        <View style={styles.machineInfo}>
          <View style={styles.machineIconContainer}>
            <Ionicons
              name={machine.os === 'darwin' ? 'laptop' : 'desktop'}
              size={24}
          ...
          <View style={styles.machineDetails}>
            ...
            <Text style={styles.machineAgents}>
              {machine.agentsDetected?.join(', ') || 'No agents detected'}
            </Text>
          </View>
          <View
            style={[
              styles.statusBadge,
              { backgroundColor: machine.isOnline ? Colors.success : Colors.textMuted },
            ]}
          >
            <Text style={styles.statusText}>{machine.isOnline ? 'Online' : 'Offline'}</Text>
          </View>
        </View>
      )}
  ...
  const ListEmpty = () => (
    <View style={styles.empty}>
      ...
      <Text style={styles.emptyText}>Start a coding agent on this machine to see it here</Text>
    </View>
  );
